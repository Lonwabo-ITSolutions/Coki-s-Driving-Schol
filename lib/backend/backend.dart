...
import 'schema/quiz_users_record.dart';
...
export 'schema/quiz_users_record.dart';
...
/// Functions to query QuizUsersRecords (as a Stream and as a Future).
Future<int> queryQuizUsersRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      QuizUsersRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<QuizUsersRecord>> queryQuizUsersRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
    ...
    queryCollection(
      QuizUsersRecord.collection,
      QuizUsersRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<QuizUsersRecord>> queryQuizUsersRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future maybeCreateUser(User user) async {
  final userRecord = QuizUsersRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await QuizUsersRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createQuizUsersRecordData(
    email: user.email,
  ...
  currentUserDocument =
      QuizUsersRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createQuizUsersRecordData(email: email));
}
